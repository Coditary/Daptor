...
use tui_debug_core::{DebugSession, print_snapshot};
            ...
            tracing_subscriber::fmt()
                .with_env_filter(
                    EnvFilter::from_default_env()
                        .add_directive("tui_debug=info".parse()?),
                )
                .init();
        }
    }
...
fn ui_binary_candidates() -> [PathBuf; 3] {
    [
        PathBuf::from("cpp/build/tui-debug-ui"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../cpp/build/tui-debug-ui"),
        PathBuf::from("tui-debug-ui"),
    ]
}
...
fn run_ui(program: &Path, mock: bool) -> Result<()> {
    ...
    #[cfg(not(unix))]
    {
        let status = command
            .status()
            .with_context(|| format!("failed to launch {}", ui.display()))?;

        if status.success() {
            Ok(())
        } else {
            bail!("tui-debug-ui exited with {}", status);
        }
    }
}
