...
use crate::dap::{spawn_adapter, spawn_debugpy_adapter, spawn_lldb_dap_adapter, DapTransport};
use crate::launch::{
    resolved_launch_from_json, resolved_launch_to_json, LaunchBackend, ResolvedLaunch,
    ResolvedUiSettings,
};
use crate::network::NetworkCapture;
use crate::terminal::{parse_run_in_terminal, DebuggeeIo, DebuggeeTerminal, LldbStdioChannels};
...
fn is_heavy_register_group(name: &str) -> bool {
    ...
    ["vector", "simd", "mmx", "sse", "avx", "neon"].iter().any(|token| lower.contains(token))
}
...
fn hex_to_bytes(hex: &str) -> Result<Vec<u8>> {
    let cleaned = hex.chars().filter(|ch| !ch.is_whitespace()).collect::<String>();
    if cleaned.is_empty() {
        return Ok(Vec::new());
    }
    if cleaned.len() % 2 != 0 {
...
fn preferred_stack_frame(stack_frames: &[StackFrame]) -> Option<&StackFrame> {
    stack_frames
        ...
        .find(|frame| {
            let path =
                frame.source.as_ref().and_then(|source| source.path.as_deref()).unwrap_or("");
...
fn parse_lldb_breakpoint_list(output: &str) -> Vec<(String, u32, u32)> {
    ...
    for line in output.lines() {
        ...
        let line_digits =
            line_body.chars().take_while(|ch| ch.is_ascii_digit()).collect::<String>();
        ...
        let hit_digits = hit_body.chars().take_while(|ch| ch.is_ascii_digit()).collect::<String>();
...
fn lldb_init_breakpoint_commands(program: &Path) -> Vec<String> {
    if let Some(source) = sibling_source_path(program) {
        return vec![format!("breakpoint set --file '{}' --name main", source.display())];
    }
...
impl DebugSession {
    ...
    pub fn launch_from_resolved(resolved: ResolvedLaunch) -> Result<Self> {
        ...
        let program = resolved.program.canonicalize().with_context(|| {
            format!("failed to resolve program path: {}", resolved.program.display())
        })?;

        info!("spawning adapter '{}' for profile '{}'", resolved.adapter_command, resolved.profile);
    ...
    pub fn terminal_write_input(&self, bytes: &[u8]) -> Result<()> {
        let mut guard = self.debuggee_io.lock().expect("debuggee io lock poisoned");
    ...
    pub fn terminal_resize(&self, rows: u16, cols: u16) {
        let guard = self.debuggee_io.lock().expect("debuggee io lock poisoned");
    ...
    fn apply_adapter_launch_extras(&mut self, launch_args: &mut Value) -> Result<()> {
        if self.adapter_ui.debugpy_launch_defaults {
            if launch_args.get("console").is_none() {
                launch_args["console"] =
                    json!(if cfg!(test) { "internalConsole" } else { "integratedTerminal" });
            }
            ...
            if let Some(extras) = adapter_launch_extras().as_object() {
                launch_args.as_object_mut().expect("launch payload object").extend(extras.clone());
            }
        }
    ...
    fn merge_capabilities_from_value(&mut self, value: &serde_json::Value) {
        ...
        if let Some(enabled) =
            value.get("supportsStepInTargetsRequest").and_then(|value| value.as_bool())
        {
            self.supports_step_in_targets = enabled;
        }
        if let Some(enabled) =
            value.get("supportsGotoTargetsRequest").and_then(|value| value.as_bool())
        {
            self.supports_goto_targets = enabled;
        }
        if let Some(enabled) = value.get("supportsStepBack").and_then(|value| value.as_bool()) {
            self.supports_step_back = enabled;
        }
        if let Some(enabled) =
            value.get("supportsDataBreakpoints").and_then(|value| value.as_bool())
        {
            self.supports_data_breakpoints = enabled;
        }
        if let Some(enabled) =
            value.get("supportsFunctionBreakpoints").and_then(|value| value.as_bool())
        {
            self.supports_function_breakpoints = enabled;
        }
        if let Some(enabled) =
            value.get("supportsCompletionsRequest").and_then(|value| value.as_bool())
        {
            self.supports_completions_request = enabled;
        }
        if let Some(enabled) =
            value.get("supportsExceptionInfoRequest").and_then(|value| value.as_bool())
        {
            self.supports_exception_info_request = enabled;
        }
        if let Some(enabled) =
            value.get("supportsReadMemoryRequest").and_then(|value| value.as_bool())
        {
            self.supports_read_memory_request = enabled;
        }
        if let Some(enabled) =
            value.get("supportsWriteMemoryRequest").and_then(|value| value.as_bool())
        {
            self.supports_write_memory_request = enabled;
        }
        if let Some(enabled) =
            value.get("supportsDisassembleRequest").and_then(|value| value.as_bool())
        {
            self.supports_disassemble_request = enabled;
        }
        if let Some(filters) =
            value.get("exceptionBreakpointFilters").and_then(|value| value.as_array())
        {
            self.exception_breakpoint_filters = filters
                ...
                .filter_map(|item| {
                    ...
                    let description = item
                        ...
                        .filter(|value| !value.is_empty());
                    let default =
                        item.get("default").and_then(|value| value.as_bool()).unwrap_or(false);
    ...
    fn handle_breakpoint_event(&mut self, body: &serde_json::Value) {
        ...
        let path = breakpoint
            ...
            .and_then(|value| value.as_str())
            .map(str::to_string);
        let id = breakpoint.and_then(|value| value.get("id")).and_then(|value| value.as_i64());
    ...
    fn snapshot_after_stop(&mut self, stopped: StoppedEventBody) -> Result<SessionSnapshot> {
        self.state =
            SessionState::Stopped { thread_id: stopped.thread_id, reason: stopped.reason.clone() };
        ...
        threads
            ...
            .filter_map(|thread| {
                self.stack_trace(thread.id)
                    .ok()
                    .map(|stack_frames| ThreadStackTrace { thread_id: thread.id, stack_frames })
            })
            .collect()
    }
    ...
    pub fn refresh_snapshot(&mut self) -> Result<SessionSnapshot> {
        let thread_id = self.active_thread.context("no active thread — session is not stopped")?;
    ...
    pub fn dispatch_continue(&mut self) -> Result<()> {
        let thread_id = self.active_thread.context("cannot continue without an active thread")?;
        let seq = self.transport.send_request("continue", json!({ "threadId": thread_id }))?;
    ...
    pub fn dispatch_step(&mut self, command: &'static str) -> Result<()> {
        ...
        let thread_id = self.active_thread.context("cannot step without an active thread")?;
        let seq = self.transport.send_request(command, json!({ "threadId": thread_id }))?;
        self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        self.state = SessionState::Running;
        Ok(())
    }
    ...
    pub fn step_in_targets(&self, frame_id: i64) -> Result<Vec<StepInTarget>> {
        ...
        let seq = self.transport.send_request("stepInTargets", json!({ "frameId": frame_id }))?;
        ...
        let parsed = serde_json::from_value::<StepInTargetsBody>(body)
            .unwrap_or(StepInTargetsBody { targets: vec![] });
        Ok(parsed.targets)
    }
    ...
    pub fn goto_targets(
        ...
        let parsed = serde_json::from_value::<GotoTargetsBody>(body)
            .unwrap_or(GotoTargetsBody { targets: vec![] });
        Ok(parsed.targets)
    }

    pub fn dispatch_goto(&mut self, target_id: i64) -> Result<()> {
        let thread_id = self.active_thread.context("cannot goto without an active thread")?;
        let seq = self
            .transport
            .send_request("goto", json!({ "threadId": thread_id, "targetId": target_id }))?;
        self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        self.state = SessionState::Running;
        Ok(())
    }
    ...
    pub fn dispatch_goto_line(&mut self, path: &str, line: i64) -> Result<()> {
        ...
        let expression =
            if path.is_empty() { format!("jump {line}") } else { format!("jump '{path}':{line}") };
    ...
    fn step(&mut self, command: &'static str) -> Result<SessionSnapshot> {
        ...
        self.poll_until_stopped()?.context("program exited during step")
    }
    ...
    fn scopes(&self, frame_id: i64) -> Result<Vec<Scope>> {
        let seq = self.transport.send_request("scopes", json!({ "frameId": frame_id }))?;
        let body = self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
    ...
    pub fn set_variable(
        ...
        let parsed = serde_json::from_value::<SetVariableBody>(body)
            .context("invalid setVariable response")?;
        Ok(Variable {
    ...
    pub fn fetch_source(&self, source_reference: i64) -> Result<String> {
        let seq = self
            .transport
            .send_request("source", json!({ "sourceReference": source_reference }))?;
        let body = self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        ...
        let parsed =
            serde_json::from_value::<SourceBody>(body).context("invalid source response")?;
        Ok(parsed.content)
    }
    ...
    pub fn read_memory(
        ...
        let parsed = serde_json::from_value::<ReadMemoryBody>(body)
            .context("invalid readMemory response")?;
        let bytes = base64::Engine::decode(
            &base64::engine::general_purpose::STANDARD,
            parsed.data.as_bytes(),
        )
    ...
    pub fn disassemble(
        ...
        let parsed = serde_json::from_value::<DisassembleBody>(body)
            .context("invalid disassemble response")?;
    ...
    pub fn set_source_breakpoints(
        ...
        let request_lines: Vec<u32> =
            breakpoints.iter().map(|breakpoint| breakpoint.line).collect();
        self.last_breakpoint_requests.insert(path_key.clone(), breakpoints.to_vec());
    ...
    pub fn data_breakpoint_info(
        ...
        let parsed = serde_json::from_value::<InfoBody>(body)
            .context("invalid dataBreakpointInfo response")?;
        let data_id = parsed
            .data_id
            .filter(|value| !value.is_empty())
    ...
    pub fn set_data_breakpoints(
    ...
    ) -> Result<Vec<DataBreakpointResult>> {
        if !self.supports_data_breakpoints {
            bail!("debug adapter does not support data breakpoints");
        }
        ...
        let seq =
            self.transport.send_request("setDataBreakpoints", json!({ "breakpoints": payload }))?;
        let body = self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        ...
        let parsed = serde_json::from_value::<BpResponse>(body)
            .context("invalid setDataBreakpoints response")?;
        Ok(parsed
            .breakpoints
            .into_iter()
            .enumerate()
            .map(|(index, bp)| {
                let data_id =
                    breakpoints.get(index).map(|entry| entry.data_id.clone()).unwrap_or_default();
    ...
    pub fn build_exception_breakpoints_request(settings: &[ExceptionBreakpointSetting]) -> Value {
        let filters: Vec<String> = settings.iter().map(|setting| setting.filter.clone()).collect();
    ...
    pub fn set_exception_breakpoints(
        ...
        let seq = self.transport.send_request("setExceptionBreakpoints", payload)?;
        self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        Ok(())
    }
    ...
    pub fn set_function_breakpoints(
        ...
        let parsed = serde_json::from_value::<BpResponse>(body)
            .context("invalid setFunctionBreakpoints response")?;
        Ok(parsed
            .breakpoints
            .into_iter()
            .enumerate()
            .map(|(index, bp)| {
                let name =
                    breakpoints.get(index).map(|entry| entry.name.clone()).unwrap_or_default();
    ...
    pub fn relaunch(&mut self) -> Result<()> {
        ...
        *self = match adapter {
            ...
            DebugAdapterKind::Generic => {
                bail!("cannot relaunch generic session without stored launch profile")
            }
        };
        Ok(())
    }
    ...
    pub fn evaluate(&self, expression: &str, frame_id: i64, context: &str) -> Result<String> {
        ...
        let timeout = if context == "watch" { WATCH_EVAL_TIMEOUT } else { REQUEST_TIMEOUT };
    ...
    pub fn completions(
        ...
        Ok(parsed
            ...
            .map(|target| {
                let selection_start =
                    target.selection_start.or(target.start).unwrap_or(0).max(0) as usize;
                let selection_length =
                    target.selection_length.or(target.length).unwrap_or(0).max(0) as usize;
                CompletionItem {
                    label: target.label,
    ...
    pub fn network_proxy_address(&self) -> Option<String> {
        self.network_capture.as_ref().map(|capture| capture.proxy_address().to_string())
    }
    ...
    pub fn step_back(&mut self) -> Result<SessionSnapshot> {
        ...
        let thread_id = self.active_thread.context("cannot step back without an active thread")?;
        let seq = self.transport.send_request("stepBack", json!({ "threadId": thread_id }))?;
        self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        self.state = SessionState::Running;
        self.poll_until_stopped()?.context("program exited during step back")
    }
    ...
    pub fn dispatch_reverse_continue(&mut self) -> Result<()> {
        ...
        let thread_id =
            self.active_thread.context("cannot reverse continue without an active thread")?;
        let seq =
            self.transport.send_request("reverseContinue", json!({ "threadId": thread_id }))?;
        self.wait_dap_response(seq, REQUEST_TIMEOUT)?;
        self.state = SessionState::Running;
        Ok(())
    }
    ...
    pub fn reverse_continue(&mut self) -> Result<SessionSnapshot> {
        ...
        self.poll_until_stopped()?.context("program exited during reverse continue")
    }
}
...
pub fn print_snapshot(snapshot: &SessionSnapshot) {
    ...
    for frame in &snapshot.stack_frames {
        let path = frame
            ...
            .unwrap_or("<unknown>");
        println!("  #{} {} @ {}:{}", frame.id, frame.name, path, frame.line);
    }
    ...
    for variable in &snapshot.variables {
        let type_name =
            variable.type_name.as_deref().map(|t| format!(" ({})", t)).unwrap_or_default();
        println!("  {} = {}{}", variable.name, variable.value, type_name);
    }
}
...
mod lifecycle_tests {
    ...
    fn inject_network_proxy_env_merges_existing_launch_env() {
        ...
        let env = launch_args.get("env").and_then(Value::as_object).expect("launch env");
    ...
    fn step_into_python_stdlib_source() {
        ...
        let top = stepped.stack_frames.first().expect("stack frame after step into");
        let path = top.source.as_ref().and_then(|source| source.path.as_deref()).unwrap_or("");
        assert!(
    ...
    fn restart_while_stopped_after_step() {
        ...
        let restarted = session
            ...
            .or_else(|| session.refresh_snapshot().ok())
            .expect("snapshot after restart");
        let restarted_line = restarted.stack_frames.first().map(|frame| frame.line).unwrap_or(0);
    ...
    fn evaluate_watch_then_continue_with_breakpoint() {
        ...
        session
            .set_source_breakpoints(
                &program,
                &[SourceBreakpoint { line: 10, condition: None, hit_condition: None }],
            )
            .expect("set breakpoint");

        let snapshot = session.refresh_snapshot().expect("initial snapshot");
        let frame_id = snapshot.stack_frames.first().map(|frame| frame.id).expect("frame id");
        session.evaluate("1 + 1", frame_id, "watch").expect("watch evaluate");
    ...
    fn disconnect_restart_and_terminate_are_safe() {
        ...
        let restarted = session
            ...
            .or_else(|| session.refresh_snapshot().ok())
            .expect("snapshot after restart");
        assert!(matches!(restarted.state, SessionState::Stopped { .. } | SessionState::Running));
...
"#;
        ...
        assert_eq!(
            parsed,
            vec![("/tmp/reverse_demo.c".into(), 15, 3), ("/tmp/reverse_demo.c".into(), 22, 1),]
        );
    }
}
...
mod lldb_launch_tests {
    ...
    #[test]
    #[ignore = "integration: requires lldb-dap"]
    fn launch_native_reverse_demo_stops_in_main() {
        ...
        for frame in &snap.stack_frames {
            let path = frame.source.as_ref().and_then(|s| s.path.as_deref()).unwrap_or("");
    ...
    fn launch_native_step_over_reverse_demo() {
        ...
        assert!(!snap.capabilities.supports_step_back, "unexpected step-back support on this host");
        let err = session
    ...
    fn launch_native_interactive_demo_accepts_fifo_stdin() {
        ...
        session.terminal_write_input(b"Alice\n").expect("write stdin fifo");
        ...
        session.shutdown().expect("shutdown");
        assert!(saw_followup, "expected fgets to accept fifo stdin and continue to next prompt");
    }
}
...
mod exception_breakpoint_request_tests {
    ...
    fn build_exception_breakpoints_request_without_conditions() {
        let payload = DebugSession::build_exception_breakpoints_request(&[
            ExceptionBreakpointSetting { filter: "cxx-throw".to_string(), condition: None },
            ExceptionBreakpointSetting { filter: "cxx-catch".to_string(), condition: None },
        ]);
        assert_eq!(
            payload,
            json!({
    ...
    fn build_exception_breakpoints_request_with_filter_options() {
        let payload =
            DebugSession::build_exception_breakpoints_request(&[ExceptionBreakpointSetting {
