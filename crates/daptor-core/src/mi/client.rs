...
impl MiClient {
    pub fn spawn() -> Result<Self> {
        ...
        Ok(Self { child, writer: BufWriter::new(stdin), reader: BufReader::new(stdout), token: 0 })
    }
    ...
    pub fn stack_locals(&mut self, thread_id: i64, frame_level: i64) -> Result<Vec<MiVariable>> {
        let response = self
            .command(&format!("-stack-list-locals --thread {thread_id} --frame {frame_level} 2"))?;
        if response.contains("^error") {
    ...
    fn exec_until_stopped(&mut self, exec_cmd: &str) -> Result<MiStopReason> {
        ...
        while Instant::now() < deadline {
            ...
            if response.starts_with('^') && response.contains("error") {
                let msg =
                    extract_quoted_field(&response, "msg").unwrap_or_else(|| response.clone());
...
fn parse_stopped(line: &str) -> Option<MiStopReason> {
    ...
    let thread_id =
        extract_quoted_field(line, "thread-id").and_then(|value| value.parse().ok()).unwrap_or(1);
...
fn parse_frames(response: &str) -> Vec<MiFrame> {
    ...
    for chunk in response.split("frame=") {
        ...
        let path = extract_quoted_field(chunk, "fullname")
            ...
            .unwrap_or_default();
        let line =
            extract_quoted_field(chunk, "line").and_then(|value| value.parse().ok()).unwrap_or(0);
        frames.push(MiFrame { level, name, path, line });
    }
...
fn parse_variables(response: &str) -> Vec<MiVariable> {
    ...
    for chunk in response.split('{') {
        ...
        variables.push(MiVariable { name, value, type_name });
    }
    variables
}
...
mod tests {
    ...
    fn parse_locals_list() {
        let response =
            r#"7^done,locals=[{name="x",type="int",value="10"},{name="y",type="int",value="32"}]"#;
