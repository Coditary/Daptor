...
pub fn default_config_directory() -> PathBuf {
    if let Ok(dir) = std::env::var("XDG_CONFIG_HOME") {
        if !dir.is_empty() {
            return PathBuf::from(dir).join("daptor");
        }
    }
    dirs_home().join(".config").join("daptor")
}
...
pub fn load_launch_config(config_path: Option<&Path>) -> Result<LaunchConfigFile> {
    ...
    let config_dir =
        config_path.parent().map(Path::to_path_buf).unwrap_or_else(default_config_directory);
    ...
    let mut config = if definitions_path.is_file() {
        ...
        bail!(
            "no launch definitions found.\n\
             ...
             See the config/definitions.yaml template in the repository.",
...
pub fn bundled_definitions_template_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../config/definitions.yaml")
}
