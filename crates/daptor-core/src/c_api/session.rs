...
impl CSession {
    pub fn launch(program: &str, args: &[String]) -> Result<Self> {
        ...
        Ok(Self { inner: SessionEngine::Dap(inner) })
    }
    ...
    pub fn launch_rr(program: &str, args: &[String]) -> Result<Self> {
        ...
        Ok(Self { inner: SessionEngine::Rr(inner) })
    }
    ...
    pub fn launch_resolved(resolved: ResolvedLaunch) -> Result<Self> {
        ...
        let inner = match resolved.backend {
            ...
            LaunchBackend::Rr => SessionEngine::Rr(RrDebugSession::launch_with_args(
                &resolved.program,
                &resolved.program_args,
            )?),
        };
        Ok(Self { inner })
    }
        ...
        match &mut self.inner {
            ...
            SessionEngine::Rr(_) => {
                let exchange = crate::network::send_compose_request(
                    None, method, url, headers, body, timeout_ms,
                )?;
    ...
    pub fn command(&mut self, cmd_json: &str) -> Result<()> {
        let cmd: CommandRequest = serde_json::from_str(cmd_json).context("invalid command JSON")?;
        ...
        match &mut self.inner {
            SessionEngine::Dap(session) => match cmd.op.as_str() {
                ...
                "goto" => {
                    let target_id = cmd.target_id.context("goto requires target_id")?;
    ...
    pub fn data_breakpoint_info(
        ...
        match &mut self.inner {
            ...
            SessionEngine::Rr(_) => {
                anyhow::bail!("data breakpoints are not supported in rr sessions")
            }
        }
    }
    ...
    pub fn set_exception_breakpoints(
        ...
        match &mut self.inner {
            ...
            SessionEngine::Rr(_) => {
                anyhow::bail!("exception breakpoints are not supported in rr sessions")
            }
        }
    }
        ...
        match &mut self.inner {
            SessionEngine::Dap(session) => {
                ...
                let json = results
                    ...
                    .collect::<Vec<_>>();
                serde_json::to_string(&json)
                    .context("failed to serialize function breakpoint results")
            }
            SessionEngine::Rr(_) => {
                anyhow::bail!("function breakpoints are not supported in rr sessions")
            }
        }
    }
    ...
    pub fn set_variable(
        ...
        let variable = match &self.inner {
            SessionEngine::Dap(session) => {
                session.set_variable(variables_reference, name, value)?
            }
    ...
    pub fn disassemble_json(
        ...
        let instructions = match &self.inner {
            SessionEngine::Dap(session) => session.disassemble(
                memory_reference,
                instruction_offset,
                offset,
                instruction_count,
    ...
    pub fn fetch_goto_targets_json(
        ...
        let targets = match &self.inner {
            ...
            SessionEngine::Rr(session) => {
                session.goto_targets(path, line, column, source_reference)?
            }
        };
...
mod tests {
    ...
    fn snapshot_json_envelope() {
        let snapshot = SessionSnapshot {
            state: SessionState::Stopped { thread_id: 1, reason: "entry".into() },
            threads: vec![],
            stack_frames: vec![],
            thread_stacks: vec![],
            scopes: vec![],
