...
use super::language_id::{
    highlight_language_candidates, normalize_language as normalize_language_id,
};
...
pub fn default_tree_sitter_dir() -> PathBuf {
    ...
    std::env::var("DAP_TREE_SITTER_DIR").map(PathBuf::from).unwrap_or_else(|_| {
...
fn state() -> &'static Mutex<TreeSitterState> {
    STATE.get_or_init(|| {
        Mutex::new(TreeSitterState { languages: HashMap::new(), runtimes: HashMap::new() })
    })
}
...
fn ensure_runtime(language: &str) -> Result<()> {
    ...
    let loaded = guard.languages.get(language).context("tree-sitter language not registered")?;
    ...
    guard.runtimes.insert(language.to_string(), LanguageRuntime { configuration });
    Ok(())
}
...
pub fn language_is_loaded(language: &str) -> bool {
    ...
    state().lock().expect("tree-sitter state lock").languages.contains_key(language)
}
...
fn highlight_source_for_language(language: &str, source: &str) -> Option<Vec<StyledSpan>> {
    ...
    let events =
        highlighter.highlight(&runtime.configuration, source.as_bytes(), None, |_| None).ok()?;
...
pub fn parse_snippet(language: &str, source: &str) -> Result<Tree> {
    ...
    parser.parse(source, None).context("parse source snippet")
}
...
fn push_span(spans: &mut Vec<StyledSpan>, text: &str, kind: HighlightKind) {
    ...
    spans.push(StyledSpan { text: text.to_string(), kind });
}

fn highlight_to_kind(highlight: Highlight) -> HighlightKind {
    let name = HIGHLIGHT_NAMES.get(highlight.0 as usize).copied().unwrap_or("variable");
