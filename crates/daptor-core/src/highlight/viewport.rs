...
pub fn highlight_viewport(
    ...
    if let Some(spans) = tree_sitter::highlight_source(language, source) {
        ...
        return all_lines
            .into_iter()
            .filter(|line| {
                line.line_number >= first_line && line.line_number < first_line + line_count
            })
            .collect();
    }
    ...
    lines[start_idx..end_idx]
        ...
        .map(|(offset, line)| HighlightedLine {
            ...
            spans: vec![StyledSpan { text: (*line).to_string(), kind: HighlightKind::Default }],
        })
        .collect()
}
...
fn spans_to_lines(
    ...
    out.push(HighlightedLine { line_number: current_line, spans: current_spans });

    while out.len() < expected_line_count {
        out.push(HighlightedLine { line_number: first_line + out.len() as i64, spans: Vec::new() });
    }
...
mod tests {
    ...
    fn highlights_python_viewport_keywords() {
        ...
        assert!(lines[0].spans.iter().any(|span| span.kind == HighlightKind::Keyword));
        assert!(lines[2].spans.iter().any(|span| span.text.contains("return")));
    }
    ...
    fn highlights_mid_file_viewport_with_surrounding_context() {
        ...
        assert!(lines[0].spans.iter().any(|span| span.kind == HighlightKind::String));
    }
