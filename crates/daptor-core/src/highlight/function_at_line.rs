...
fn identifier_from_declarator(node: Node, source: &[u8]) -> Option<String> {
    match node.kind() {
        ...
        | "destructor_name" => node_name_text(node, source),
        "pointer_declarator"
...
mod tests {
    ...
    fn python_function_name_at_definition_line() {
        ...
        assert_eq!(function_name_at_line("python", source, 1), Some("foo".to_string()));
        assert_eq!(function_name_at_line("python", source, 4), Some("bar".to_string()));
        assert_eq!(function_name_at_line("python", source, 2), None);
        assert_eq!(function_definition_line("python", source, "bar"), Some(4));
    }
    ...
    fn c_function_name_uses_builtin_grammar() {
        ...
        assert_eq!(function_name_at_line("c", source, 1), Some("add".to_string()));
    }
}
