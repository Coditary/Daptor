use daptor_core::highlight::{highlight_viewport, init, language_from_path, HighlightKind};
...
fn builtin_languages_highlight_representative_syntax() {
    ...
    for (language, source) in samples {
        ...
        assert!(count >= 4, "expected rich highlighting for {language}, got {count} colored spans");
    }
}
