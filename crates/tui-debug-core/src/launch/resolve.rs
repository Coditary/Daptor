...
use super::config::{
    AdapterUiSettings, LaunchConfigFile, PathMapEntry, ProfileBackend, ProfileDefinition,
    load_launch_config,
};
...
pub fn resolve_launch(
    ...
    let launch =
        merge_launch_payload(&adapter.launch, &profile.launch, &profile.path_map, &ctx, &program, &request.args);
    let initialize = merge_initialize_payload(&adapter.initialize);
    let ui = ResolvedUiSettings::from(adapter.ui);

    let display_source = if target.is_file() && !is_elf_executable(&target) {
        Some(target.clone())
    } else {
        None
    };
...
fn build_context(target: &Path, workspace: &Path, binary_override: Option<PathBuf>) -> LaunchResolveContext {
    let package = if target.extension().is_some_and(|ext| ext == "go") || find_nearest_file(target, "go.mod").is_some() {
...
fn apply_profile_resolve(ctx: &mut LaunchResolveContext, resolve: &HashMap<String, String>) {
    for (key, template) in resolve {
        ...
        ctx.values.insert(key.clone(), value.clone());
        match key.as_str() {
...
fn merge_initialize_payload(adapter_initialize: &Value) -> Value {
    ...
    if let Some(object) = adapter_initialize.as_object() {
        payload
            .as_object_mut()
            .expect("initialize payload object")
            .extend(object.clone());
    }
...
fn merge_launch_payload(
    ...
    launch.insert(
        "program".to_string(),
        Value::String(program.display().to_string()),
    );
    if !args.is_empty() {
    ...
    if !launch.contains_key("cwd") {
        launch.insert(
            "cwd".to_string(),
            Value::String(ctx.workspace.display().to_string()),
        );
    }
...
fn expand_value_strings(mut value: Map<String, Value>, ctx: &LaunchResolveContext) -> Map<String, Value> {
...
fn evaluate_placeholder(placeholder: &str, ctx: &LaunchResolveContext) -> String {
    if let Some((func, arg)) = placeholder.split_once(':') {
        ...
        return match func {
            ...
            "binary_probe" => resolve_native_launch_path(Path::new(&expanded_arg))
                .display()
                .to_string(),
            "go_package" => go_package_dir(Path::new(&expanded_arg))
                .display()
                .to_string(),
