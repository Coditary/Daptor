...
/// If `path` is a source file, return a sibling executable when it exists.
pub fn sibling_executable_for_source(path: &Path) -> Option<PathBuf> {
    let path_str = path.to_string_lossy();
    for extension in SOURCE_EXTENSIONS {
        if path_str.ends_with(extension) {
            let without = PathBuf::from(path_str.trim_end_matches(extension));
            if is_elf_executable(&without) {
                return Some(without);
            }
            if without.exists() && without.is_file() {
                return Some(without);
            }
        }
    }
...
/// Resolve a native launch path: ELF passthrough, else sibling executable from source stem.
