...
use crate::rr::{ensure_gdb_available, ensure_rr_available, pick_replay_port, record_program, spawn_replay_server};
use crate::session::{
...
impl RrDebugSession {
    ...
    pub fn launch_with_args(program: impl AsRef<Path>, args: &[String]) -> Result<Self> {
        ...
        info!(
            "rr replay session ready on port {} for {}",
            replay_port,
    ...
    pub fn set_source_breakpoints(
        ...
        for breakpoint in breakpoints {
            self.mi
                .insert_breakpoint(&path.to_string_lossy(), breakpoint.line)?;
        }
        Ok(())
    }
    ...
    pub fn step_in_targets(&self, _frame_id: i64) -> Result<Vec<crate::dap::protocol::StepInTarget>> {
        Ok(Vec::new())
    }
    ...
    fn refresh_snapshot(&mut self) -> Result<SessionSnapshot> {
        ...
        let thread_stacks = vec![ThreadStackTrace {
            thread_id,
