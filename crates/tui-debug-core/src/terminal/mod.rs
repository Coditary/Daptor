...
impl LldbStdioChannels {
    pub fn open() -> Result<Self> {
        let unique = SystemTime::now()
            ...
            .unwrap_or(0);
        let stdin_fifo_path = std::env::temp_dir().join(format!("tui-debug-stdin-{unique}"));
    ...
    pub fn write_input(&mut self, bytes: &[u8]) -> Result<()> {
        ...
        let normalized: Vec<u8> = bytes.iter().map(|byte| if *byte == b'\r' { b'\n' } else { *byte }).collect();
...
impl DebuggeeTerminal {
    ...
    pub fn spawn_stopped(args: &[String], cwd: Option<&str>, env: &[(String, String)]) -> Result<Self> {
    ...
    pub fn write_input(&mut self, bytes: &[u8]) -> Result<()> {
        ...
        self.master
            .write_all(bytes)
            .context("failed to write to debuggee PTY")?;
...
fn child_main(args: &[String], cwd: Option<&str>, env: &[(String, String)]) {
    ...
    if let Some(dir) = cwd {
        if std::env::set_current_dir(dir).is_err() {
            eprintln!("tui-debug: failed to chdir to {dir}");
        }
    }
...
pub fn parse_run_in_terminal(body: &Value) -> Result<(Vec<String>, Option<String>, Vec<(String, String)>)> {
    let args = body
        .get("args")
        ...
        .map(|value| {
            value
                .as_str()
                .context("runInTerminal args must be strings")
                .map(str::to_string)
        })
