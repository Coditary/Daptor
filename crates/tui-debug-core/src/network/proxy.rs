...
use hudsucker::{
    ...
    rcgen::{BasicConstraints, CertificateParams, DistinguishedName, DnType, IsCa, Issuer, KeyPair},
...
pub fn start_proxy_server(inner: Arc<CaptureInner>) -> anyhow::Result<()> {
    ...
    std::thread::Builder::new()
        .name("tui-debug-network-proxy".into())
        .spawn(move || {
...
fn write_ca_files(ca_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    params
        .distinguished_name
        .push(DnType::CommonName, "tui-debug network capture");
