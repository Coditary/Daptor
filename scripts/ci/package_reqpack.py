...
def build_metadata(
    ...
    return {
        ...
        "release": 1,
        "revision": 0,
        "summary": "Terminal debugger (DAP + Tuinator UI)",
        "description": "Daptor pairs a Rust DAP engine with a terminal UI for Python, native, and rr-backed debugging.",
        "license": "MIT",
        "architecture": arch,
        ...
        "tags": ["cli", "debugger", "dap", "tui", "daptor"],
        "payload": {
...
def stage_payload(
    ...
    payload_config_dir = payload_root / "share" / "daptor" / "config"
    payload_schema_dir = payload_root / "share" / "daptor" / "schemas"
    ...
    shutil.copy2(ui_binary, payload_bin_dir / PRODUCT)
    shutil.copy2(cli_binary, payload_bin_dir / "daptor-cli")
    copy_runtime_files(ui_binary, payload_bin_dir, platform)
    copy_tree(repo_root / "config", payload_config_dir)
    copy_tree(repo_root / "schemas", payload_schema_dir)
    ...
    if platform == "linux":
        ...
        bundle_linux_libraries(payload_bin_dir / PRODUCT, lib_dir, repo_root)
        bundle_linux_libraries(payload_bin_dir / "daptor-cli", lib_dir, repo_root)


def main() -> int:
    parser = argparse.ArgumentParser(description="Package daptor as ReqPack .rqp archive")
    ...
    with tempfile.TemporaryDirectory(prefix=f"daptor-rqp-{args.platform}-{args.arch}-", dir=output_dir) as temp_dir:
        ...
        stage_payload(payload_root, ui_binary, cli_binary, repo_root, args.platform)
