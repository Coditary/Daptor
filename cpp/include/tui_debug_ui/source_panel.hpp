...
class SourcePanel : public tuinator::Widget {
  public:
    ...
    const std::unordered_map<int, bool>& breakpoints() const { return breakpoints_; }
    void set_breakpoints(std::unordered_map<int, bool> breakpoints);
  ...
  private:
    ...
    std::unordered_map<int, bool> breakpoints_;
