...
class MemoryPanel {
  public:
    ...
    void set_search_highlight(std::size_t byte_offset, std::size_t length);
    void clear_search_highlight();
    void set_search_match_counter(int current, int total);
    void clear_search_match_counter();
    void focus_address_input();
    void focus_search_input();
    void blur_toolbar_inputs();
    void cancel_toolbar_inputs();
    ...
    [[nodiscard]] bool is_toolbar_text_input_focused() const;
    ...
    void set_on_search_change(std::function<void(const std::string&)> callback);
    ...
    void set_on_toolbar_cancel(std::function<void()> callback);
    void set_on_row_selected(std::function<void(int row)> callback);
    ...
    bool handle_list_navigation_key(const tuinator::Event& event);
    bool handle_list_activate_key(const tuinator::Event& event);
  ...
  private:
    ...
    tuinator::Widget* search_match_label_ = nullptr;
    ...
    std::function<void(const std::string&)> on_search_change_;
    std::function<void()> on_inline_edit_cancel_;
    std::function<void()> on_toolbar_interact_;
    std::function<void()> on_toolbar_cancel_;
    std::function<void(int row)> on_row_selected_;
    std::string address_cancel_value_;
    std::string search_cancel_value_;
};
